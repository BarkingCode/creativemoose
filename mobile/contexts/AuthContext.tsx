...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    if (errorMsg.includes("signups not allowed") || errorMsg.includes("user not found") || errorMsg.includes("otp")) {
      const { error: createError } = await supabase.auth.signInWithOtp({
        ...
        options: {
          ...
          emailRedirectTo: "kg-photo://auth/callback",
        },
      });
  ...
  const linkWithApple = async (): Promise<{ error: Error | null }> => {
    ...
    try {
      ...
      // Note: linkIdentity with native Apple requires using updateUser approach
      // For now, we'll use the ID token method
      if (error) {
        // Fallback: Try signing in with Apple and merging data manually if needed
