/**
 * Generate Page (Tabs Version)
 *
 * Camera view and AI image generation for authenticated users.
 * Essentially the same as the original /generate page but within tabs layout.
 */
...
export default function GeneratePage() {
  ...
  const { stream, videoRef, canvasRef, stopCamera, restartCamera } =
    useCamera(isMounted);
  const { credits, freeCredits, videoCredits, videoFreeCredits, fetchCredits } =
    useCredits(isSignedIn);
  const {
    generating,
    results,
    videos,
    ...
    handleGenerate,
    resetGeneration,
  } = useImageGeneration(isSignedIn, fetchCredits);
  ...
  const hasImageCredits =
    freeCredits > 0 || (credits !== null && credits > 0);
  const hasVideoCredits =
    videoFreeCredits > 0 || (videoCredits !== null && videoCredits > 0);
  const hasCredits =
    selectedPresetType === "video" ? hasVideoCredits : hasImageCredits;
